...
enum UserRole { homeOwner, worker }
...
abstract class AuthRepository extends ChangeNotifier {
  ...
  Future<void> registerHomeOwner(String fullName);
...
class SupabaseAuthRepository extends ChangeNotifier implements AuthRepository {
  ...
  @override
  Future<void> registerHomeOwner(String fullName) async {
    ...
    notifyListeners();
    try {
      await _client.from('home_owners').insert({
        'id': _userId,
        'full_name': fullName,
        'phone_number': _phoneNumber,
      });
      _role = UserRole.homeOwner;
      _hasProfile = true;
    } finally {
      _isLoading = false;
      notifyListeners();
  ...
  Future<void> checkExistingProfile() async {
    ...
    notifyListeners();
    try {
      // 1. Check if user is a Homeowner
      final homeownerRes = await _client
          .from('home_owners')
          .select()
          .eq('id', _userId!)
          .maybeSingle();
      
      if (homeownerRes != null) {
        _role = UserRole.homeOwner;
        _hasProfile = true;
        return;
      }
...
class MockAuthRepository extends ChangeNotifier implements AuthRepository {
  ...
  static final Map<String, String> _mockHomeowners = {}; // id -> fullName
  static final Map<String, Map<String, String>> _mockWorkers = {}; // id -> {fullName, fayda}
  ...
  Future<void> registerHomeOwner(String fullName) async {
    ...
    _mockHomeowners[_userId!] = fullName;
    _role = UserRole.homeOwner;
    _hasProfile = true;
  ...
  Future<void> checkExistingProfile() async {
    ...
    if (_mockHomeowners.containsKey(_userId)) {
      _role = UserRole.homeOwner;
      _hasProfile = true;
