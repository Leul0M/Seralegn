...
class _ClientDashboardState extends State<ClientDashboard> {
  ...
  Future<void> _loadJobs() async {
    ...
    try {
      final jobs = await dbRepo.getClientJobs(authRepo.currentUserId!);
      setState(() {
        _jobs = jobs;
      });
  ...
  void _confirmJobCompleted(JobModel job) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Failed to confirm job: $e')));
      }
    }
  }
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                final dbRepo = Provider.of<DatabaseRepository>(
                  context,
                  listen: false,
                );
                try {
                  ...
                  if (context.mounted) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text(
                          'Issue reported. Support will review this case shortly.',
                        ),
  ...
  Widget _buildStatusBadge(String status) {
    ...
    switch (status) {
      ...
      case 'pending_confirmation':
        ...
        break;
      case 'completed':
        bg = AppTheme.actionGreen.withValues(alpha: 0.1);
                ...
                child: Row(
                  children: [
                    CircleAvatar(
                      ...
                      backgroundImage: workerImage != null
        ...
        child: Column(
          ...
          children: [
            ...
            _buildProfileDetailRow(
              Icons.person,
              'Full Name',
              authRepo.hasProfile ? 'Registered User' : 'Loading...',
            ),
            const Divider(),
            _buildProfileDetailRow(
              Icons.phone,
              'Phone Number',
              authRepo.currentPhoneNumber ?? 'Unknown',
            ),
            const Divider(),
            _buildProfileDetailRow(
              Icons.verified_user,
  ...
  Widget? _buildPendingConfirmationOverlay() {
    ...
    final worker = pendingJob.workerId != null
        ? _workerCache[pendingJob.workerId]
        : null;
