...
class SupabaseDatabaseRepository implements DatabaseRepository {
  ...
  @override
  Future<ClientModel?> getClientProfile(String id) async {
    try {
      ...
      if (kDebugMode) print('Error getClientProfile: $e');
      rethrow;
    }
  }
  ...
  Future<List<JobModel>> getClientJobs(String clientId) async {
    try {
      ...
      if (kDebugMode) print('Error getClientJobs: $e');
      rethrow;
    }
  }
  ...
  Future<void> renewSubscription({
    ...
    try {
      ...
      await _client
          .from('workers')
          .update({'subscription_expires_at': newExpiry.toIso8601String()})
          .eq('id', workerId);
    } catch (e) {
...
class MockDatabaseRepository implements DatabaseRepository {
  ...
  static final Map<String, ClientModel> _mockClients = {};
    ...
    if (_mockJobs.isEmpty) {
      _mockJobs.addAll([
        JobModel(
          ...
          category: 'Plumbing',
          description:
              'Looking for someone to fix a sink in my kitchen. It\'s been dripping continuously and seems to have a faulty connector.',
          offeredPrice: 400.00,
          status: 'open',
          locationLat: 9.0300,
          locationLng: 38.7400,
        ...
        JobModel(
          ...
          category: 'Cleaning',
          description:
              'Move-out cleaning required for an empty 2 bedroom apartment. Focus on kitchen and bathrooms.',
          offeredPrice: 600.00,
          status: 'open',
          locationLat: 9.0120,
          locationLng: 38.7520,
        ...
        JobModel(
          ...
          category: 'Electrical',
          description:
              'Need a qualified electrician to install a new ceiling fan in the living room. Wiring is already present from the previous fan.',
          offeredPrice: 850.00,
          status: 'open',
          locationLat: 9.0220,
          locationLng: 38.7650,
      ...
      // Seed dummy profiles
      _mockClients['mock-ho-1'] = ClientModel(
        id: 'mock-ho-1',
        fullName: 'Almaz Abebe',
        phoneNumber: '+251911111111',
      );
      _mockClients['mock-ho-2'] = ClientModel(
        id: 'mock-ho-2',
        fullName: 'Kassahun Tilahun',
        phoneNumber: '+251922222222',
      );
      _mockClients['mock-ho-3'] = ClientModel(
        id: 'mock-ho-3',
        fullName: 'Fasil Kebede',
  ...
  Future<void> createJob(JobModel job) async {
    ...
    // Ensure client exists in our mock data
    if (!_mockClients.containsKey(job.clientId)) {
      _mockClients[job.clientId] = ClientModel(
        id: job.clientId,
  ...
  Future<List<JobModel>> getClientJobs(String clientId) async {
    ...
    // Seed initial dashboard jobs specifically for this client to match mockup
    final userJobs = _mockJobs.where((e) => e.clientId == clientId).toList();
    if (userJobs.isEmpty) {
      _mockJobs.addAll([
        JobModel(
          ...
          category: 'Plumbing',
          description:
              'The kitchen sink tap has a steady drip. Needs a plumber to repair or replace the washer.',
          offeredPrice: 450.00,
          status: 'in_progress',
          locationLat: 9.0300,
          locationLng: 38.7400,
        ...
        JobModel(
          ...
          category: 'Handyman',
          description:
              'Need to mount a 55-inch TV on a concrete wall. Bracket is already purchased.',
          offeredPrice: 900.00,
    ...
    return _mockJobs.where((e) => e.clientId == clientId).toList()..sort(
  ...
  Future<List<JobModel>> getOpenJobs() async {
    await Future.delayed(const Duration(milliseconds: 200));
    return _mockJobs.where((e) => e.status == 'open').toList()..sort(
      (a, b) => (b.createdAt ?? DateTime.now()).compareTo(
        a.createdAt ?? DateTime.now(),
      ),
    );
  ...
  Future<void> renewSubscription({
    ...
    if (worker != null) {
      _mockWorkers[workerId] = worker.copyWith(
        ...
        trialEndsAt: DateTime.now().subtract(
          const Duration(days: 1),
        ), // Ensure trial is past
      );
    }
  }
